...
from pseudo.dataset.augs_ALIA import cut_mix_label_adaptive, cut_mix, cut_out, cut_mix_label_inject, rotate, Perspective, Affine, random_strong_aug
...
from pseudo.utils.loss_helper import get_criterion, compute_unsupervised_loss_by_threshold
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            # strong_aug_list = [rotate, Perspective, Affine, cut_mix, cut_out]
            # strong_ops = random.choices(strong_aug_list, k = cfg["trainer"]["unsupervised"].get("random_strong_aug", 1))
            # for op in strong_ops:
            #     if getattr(op, '__name__') == 'cut_mix_label_adaptive':
            #         image_u_aug, pseudo_label, pseudo_confid = op(
            #                 image_u_aug,
            #                 pseudo_label,
            #                 pseudo_confid, 
            #                 image_l,
            ...
            #     elif getattr(op, '__name__') == 'cut_mix_label_inject':
            #         image_u_aug, pseudo_label, pseudo_confid = op(
            #                 image_u_aug,
            #                 pseudo_label,
            #                 pseudo_confid, 
            ...
            #             )
            #     else:
            #         image_u_aug, pseudo_label, pseudo_confid = op(image_u_aug, pseudo_label, pseudo_confid)
