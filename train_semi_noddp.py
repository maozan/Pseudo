...
from pseudo.dataset.builder_noddp import get_loader
from pseudo.models.model_helper_noddp import ModelBuilder
...
from pseudo.utils.utils_noddp import AverageMeter, intersectionAndUnion, load_state
from pseudo.utils.utils_noddp import init_log, set_random_seed, setup_default_logging
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        sup_losses.update(sup_loss.clone().detach().item())

        uns_losses.update(unsup_loss.clone().detach().item())

        meter_high_pseudo_ratio.update(pseduo_high_ratio.clone().detach().item())
