...
from pseudo.utils.loss_helper import get_criterion, compute_unsupervised_loss_by_threshold, compute_unsupervised_loss_softmatch
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            pseduo_high_ratio = torch.tensor(0.0).cuda()
            unsup_loss = compute_unsupervised_loss_softmatch(pred_u_strong, pseudo_label.detach(), pseudo_confid.detach())
