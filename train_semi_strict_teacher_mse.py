...
from pseudo.utils.loss_helper import get_criterion, mse_loss, compute_unsupervised_loss_by_threshold
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            unsup_loss_1 = mse_loss(inputs=pred_u_strong, targets=pred_t.detach())
            unsup_loss_2 = mse_loss(inputs=pred_u_strong, targets=pred_ta.detach())
        ...
        loss = sup_loss + unsup_loss + 0.2 * unsup_loss_1 + 0.1 * unsup_loss_2
