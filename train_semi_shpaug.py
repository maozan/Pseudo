...
from pseudo.utils.loss_helper import get_criterion, compute_unsupervised_loss_by_threshold, compute_unsupervised_loss
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        _, image_u_weak, image_u_intense, _ = loader_u_iter.next()
        # weak: weak aug, aug: intensity-base aug
        image_u_weak, image_u_intense = image_u_weak.cuda(), image_u_intense.cuda()
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            if cfg["trainer"]["unsupervised"].get("use_strong", False):
                if cfg["trainer"]["unsupervised"].get("use_cutmix_adaptive", False):
                    ...
                    image_u_weak_aug, pseudo_label, pseudo_confid = cut_mix_label_adaptive(
                            image_u_weak,
                            pseudo_label,
                            pseudo_confid, 
                            image_l,
                    ...
                    # using rotate
                    image_u_weak_aug, pseudo_label, pseudo_confid = rotate(image_u_weak, pseudo_label, pseudo_confid)
                
                elif cfg["trainer"]["unsupervised"].get("use_perspective", False):
                    # using rotate
                    image_u_weak_aug, pseudo_label, pseudo_confid = Perspective(image_u_weak, pseudo_label, pseudo_confid)
                
                elif cfg["trainer"]["unsupervised"].get("use_Affine", False):
                    # using rotate
                    image_u_weak_aug, pseudo_label, pseudo_confid = Affine(image_u_weak, pseudo_label, pseudo_confid)

                elif cfg["trainer"]["unsupervised"].get("use_cutmix", False):
                    # using navie cutmix
                    image_u_weak_aug, pseudo_label, pseudo_confid, image_u_intense_aug = cut_mix(image_u_weak, pseudo_label, pseudo_confid, image_u_intense)

                elif cfg["trainer"]["unsupervised"].get("use_cutout", False):
                    # using navie cutout
                    image_u_weak_aug, pseudo_label, pseudo_confid = cut_out(image_u_weak, pseudo_label, pseudo_confid)

                elif cfg["trainer"]["unsupervised"].get("use_cutmix_labelinj", False):
                    # using cutmix_label_inject
                    image_u_weak_aug, pseudo_label, pseudo_confid = cut_mix_label_inject(
                            image_u_weak,
                            pseudo_label,
                            pseudo_confid, 
                            image_l,
            ...
            ## image_u_weak 需要进过基于形状的强增强后与pred_u做对比
            model.eval()
            pred_u_stu, _ = model(image_u_weak_aug.detach())
            pred_u_stu = F.softmax(pred_u_stu, dim=1) # 归一化
            pred_confid_stu, pred_stu = torch.max(pred_u_stu, dim=1) 

            ## 判断stu是否需要更强的增强
            pseudo_label_mask = pseudo_confid.ge(0.9).bool() * (pseudo_label != 255).bool()
            pseudo_label[~pseudo_label_mask] = 255 # 能有这个赋值操作一定要没有梯度   

            should_strong_mask = pred_stu.eq(pseudo_label).bool() * (pseudo_label != 255).bool() # 容易预测的pixel施加strong aug以增加泛化性
            should_strong_mask = should_strong_mask.unsqueeze(1).repeat(1, 3, 1, 1)
            image_u_aug = torch.where(should_strong_mask == True, image_u_intense_aug, image_u_weak_aug)
