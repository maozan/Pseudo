...
from pseudo.dataset.augs_ALIA import random_strong_aug, cut_mix
...
def train(
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            pseduo_high_ratio = torch.tensor(0.0).cuda()
            unsup_loss = compute_unsupervised_loss(pred_u_strong, pseudo_label.detach())
        ...
        loss = sup_loss + cfg["trainer"]["unsupervised"].get("loss_weight", 1.0) * unsup_loss
